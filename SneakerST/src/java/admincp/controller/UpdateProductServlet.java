...
public class UpdateProductServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            db.updateSize(p, product_size);
            db.updateImageURL(p, product_image_url);
            request.setAttribute("cp-msg", "Updated Product");
            RequestDispatcher rd = request.getRequestDispatcher("/admincp/viewproduct?id="+p.getId());
            rd.forward(request, response);
        } catch (SQLException ex) {
            request.setAttribute("cp-msg", "Add Product Failed");
            int id = (Integer.parseInt(request.getParameter("product_id")));
            Logger.getLogger(AddProductServlet.class.getName()).log(Level.SEVERE, null, ex);
            RequestDispatcher rd = request.getRequestDispatcher("/admincp/viewproduct?id="+id);
            rd.forward(request, response);
        }
    }
